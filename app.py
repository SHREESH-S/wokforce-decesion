...
# =============================================================================
# 4. SECURITY GATEWAY — RBAC & Lockout Protection
# =============================================================================
...
if st.session_state.authenticated:
    ...
    if elapsed > timedelta(minutes=SESSION_TIMEOUT_MINUTES):
        log_event("SESSION_TIMEOUT", f"Session for {st.session_state.get('username','?')} expired.")
...
def run_ai_matchmaking(apply=True):
    """Skill + geography + performance + cost aware assignment."""
...
def process_query(user_prompt):
    ...
    if any(k in prompt_l for k in ["match", "assign", "route"]):
        ...
        return f"⚡ **AI Engine Execution Complete:** {n} pending task(s) matched to best-suited candidates based on skills, geography, cost, and capacity."
    ...
    if "cost" in prompt_l or "saving" in prompt_l or "cheaper" in prompt_l:
        ...
        return (f"💰 **Top savings opportunity:** swap {top['Task_ID']} from {top['Current_Engineer']} "
                ...
                f"→ saves ${top['Est_Savings_Per_Hr']}/hr.")
    ...
    if "forecast" in prompt_l or "projection" in prompt_l or "utilization" in prompt_l:
        ...
        return f"📈 If pending tasks were routed now, **{hottest['Name']}** would peak at {hottest['Projected_Util_%']}% utilization."
...
if menu == "Dashboard Center":
    ...
    # Uses Streamlit 1.35.0+ @st.fragment for non-blocking chart renders
    @st.fragment
...
# =============================================================================
# TAB 2: AI NATURAL LANGUAGE CHATBOT
# =============================================================================
...
# =============================================================================
# TAB 4: DYNAMIC SCENARIO ENGINE
# =============================================================================
elif menu == "Dynamic Scenario Engine":
    st.markdown('<p class="app-title">🔮 Dynamic Scenario & Stress Engine</p>', unsafe_allow_html=True)
    st.markdown('<p class="app-sub">Simulate operational shock scenarios before executing changes.</p>', unsafe_allow_html=True)

    c1, c2 = st.columns(2)
    with c1:
        st.subheader("⚙️ Simulation Controls")
        workload_spike = st.slider("Simulate Task Spike (%)", 0, 200, 50, 10)
        absenteeism = st.multiselect("Simulate Unexpected Absence", st.session_state.engineers["Name"].tolist())
        run_sim = st.button("Execute Stress Test Scenario", type="primary")

    if run_sim:
        sim_eng = st.session_state.engineers.copy()
        if absenteeism:
            sim_eng.loc[sim_eng["Name"].isin(absenteeism), "Availability"] = "On Leave"

        available_cap = sim_eng[sim_eng["Availability"] == "Available"]["Max_Capacity"].sum() - sim_eng[sim_eng["Availability"] == "Available"]["Workload"].sum()
        current_pending = len(st.session_state.tasks[st.session_state.tasks["Assigned_To"].astype(str).str.contains("Unassigned")])
        projected_pending = int(current_pending * (1 + workload_spike / 100))

        with c2:
            st.subheader("📈 Simulation Metrics")
            sc1, sc2 = st.columns(2)
            sc1.metric("Available Headroom", available_cap)
            sc2.metric("Projected Pending Queue", projected_pending)

            if projected_pending > available_cap:
                st.error(f"⚠️ **Capacity Breach Warning**: Deficit of {projected_pending - available_cap} task slot(s).")
            else:
                st.success("✅ Operational headroom is sufficient to absorb scenario load.")

# =============================================================================
# TAB 5: SECURITY & AUDIT LOGS
# =============================================================================
elif menu == "Security & Audit":
    st.markdown('<p class="app-title">🛡️ Security Gateway & Audit Vault</p>', unsafe_allow_html=True)
    st.markdown('<p class="app-sub">Tamper-evident SHA-256 hash-chained system action logs.</p>', unsafe_allow_html=True)

    is_valid, breach_idx = verify_audit_chain()
    if is_valid:
        st.success("🔒 **Cryptographic Audit Integrity**: Hash chain verified (0 tampered records).")
    else:
        st.error(f"🚨 **Audit Integrity Compromised**: Hash mismatch at log entry index {breach_idx}!")

    if st.session_state.audit_logs:
        log_df = pd.DataFrame(st.session_state.audit_logs).drop(columns=["_ts_raw"])
